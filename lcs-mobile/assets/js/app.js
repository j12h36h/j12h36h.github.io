...
async function publishPost(){
  ...
  try{
    ...
    $('#composerText').value=''; if($('#postTags'))$('#postTags').value=''; renderTagPreview($('#postTags'),$('#postTagPreview')); $('#charCounter').textContent='0 / 1200'; toast('Public thought published.');
...
async function createObject(e){
  ...
  try{
    ...
    formEl.reset(); renderTagPreview($('#createTags'),$('#createTagPreview')); closeDialog('#createDialog'); toast(`${kind} created.`);
    if(related){
...
async function submitComment(e){
  ...
  try{
    ...
    $('#detailCommentText').value='';renderDetailThread();toast('Response added.');
...
let mobileGoogleSignInBusy=false;
function showAuthError(err){const code=String(err?.code||'auth/unknown');const ua=navigator.userAgent||'';const embedded=/FBAN|FBAV|Instagram|Line\/|Discord|wv\)|; wv|GSA\//i.test(ua);$('#authErrorTitle').textContent='Google sign-in needs attention';$('#authErrorText').textContent=code.includes('unauthorized-domain')?'This host is not authorized in Firebase Authentication. Add j12h36h.github.io under Authentication → Settings → Authorized domains.':code.includes('popup-closed')?'The Google window was closed before sign-in finished.':code.includes('popup-blocked')?'The browser blocked the Google sign-in window. Allow popups for this site and retry.':code.includes('cancelled-popup-request')?'A Google sign-in is already in progress. Finish or close it, then retry.':code.includes('internal-error')?(embedded?'This embedded app browser is interfering with Google sign-in. Open LCS Mobile directly in Chrome or Safari and retry.':'Firebase returned an internal mobile sign-in error. This build now avoids IndexedDB Auth persistence and uses local/session storage instead. If this still appears, test once with HTTP-referrer Application restrictions disabled on the Firebase browser key while keeping its Firebase API restrictions enabled.'):'Firebase could not complete Google sign-in. Check Authentication, authorized domains, browser key restrictions, and try Chrome or Safari directly.';$('#authErrorCode').textContent=code;$('#authErrorBox').hidden=false;}
async function signInGoogle(){clearAuthError();if(mobileGoogleSignInBusy)return;if(!state.firebase?.auth)return showAuthError({code:'auth/configuration-not-found'});const {auth,authMod}=state.firebase;const provider=new authMod.GoogleAuthProvider();provider.setCustomParameters({prompt:'select_account'});const button=$('#googleSignInButton');mobileGoogleSignInBusy=true;if(button){button.disabled=true;button.dataset.originalText=button.textContent;button.textContent='Opening Google…';}try{await authMod.signInWithPopup(auth,provider);closeDialog('#authDialog');toast('Signed in. LCS is linking a separate public identity.');}catch(e){console.error(e);showAuthError(e);}finally{mobileGoogleSignInBusy=false;if(button){button.disabled=false;button.textContent=button.dataset.originalText||'Continue with Google';delete button.dataset.originalText;}}}
...
async function initFirebase(){
  ...
  try{
    ...
    const app=appMod.initializeApp(LCS_CONFIG.firebase);let auth;try{auth=authMod.initializeAuth(app,{persistence:[authMod.browserLocalPersistence,authMod.browserSessionPersistence,authMod.inMemoryPersistence],popupRedirectResolver:authMod.browserPopupRedirectResolver});}catch(e){console.debug('Mobile Auth explicit initialization fallback',e?.code||e);auth=authMod.getAuth(app);}authMod.useDeviceLanguage(auth);state.firebase={app,auth,authMod,db:null,fsMod:null};
    ...
    setBackendStatus('LCS Mobile v0.7.10 connected','Public content uses random public profile IDs. Status authorization, soft-delete retention, and immutable moderation logs are enforced by Firestore rules.','ok');
...
function initialRender(){renderAuth();renderAccount();renderSpaces();renderFeed();renderCatalogs();renderCommunities();renderTrends();renderLfg();renderConnections();renderStatusSurfaces();renderModeration();setView(location.hash.replace('#','')||'home',false);}

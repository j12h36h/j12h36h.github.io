...
function renderFriendMessageRow(profileId, extra='Friend') { return `<div class="request-row">${renderPersonRow(profileId,extra)}<div class="request-actions"><button class="primary-button" data-message-profile="${escapeHtml(profileId)}" type="button">Message</button></div></div>`; }
function renderConnections() {
  ...
  const friends=[...new Set(friendProfileIds())]; $('#friendCountBadge').textContent=String(friends.length); $('#friendList').innerHTML=friends.length?friends.map(id=>renderFriendMessageRow(id,'Friend')).join(''):'<p class="muted">No accepted friends yet.</p>';
...
async function initFirebase(){
  ...
  try{
    ...
    const app=appMod.initializeApp(LCS_CONFIG.firebase);const auth=authMod.getAuth(app);try{await authMod.setPersistence(auth,authMod.browserLocalPersistence);}catch(e){console.debug('Mobile Auth local persistence fallback',e?.code||e);}authMod.useDeviceLanguage(auth);state.firebase={app,auth,authMod,db:null,fsMod:null};
    try{const redirectResult=await authMod.getRedirectResult(auth);if(redirectResult?.user){sessionStorage.removeItem('lcsMobileAuthReturn');closeDialog('#authDialog');toast('Signed in. LCS is linking a separate public identity.');}}catch(e){console.error('Mobile redirect sign-in failed',e);sessionStorage.removeItem('lcsMobileAuthReturn');showAuthError(e);}
    authMod.onAuthStateChanged(auth,user=>{const next=user?.uid||null;const changed=next!==state.authUid;state.authUid=next;state.authReady=true;if(changed){state.profileId=null;state.publicProfile=null;state.creditBalance=0;state.profileVerified=false;state.accountDirty=false;state.profileSaveStatus=next?'Authentication verified · creating private identity link…':'';state.legacyMigrationStarted=false;state.founderBootstrapAttempted=false;state.statusPublic=[];state.statusOwn=[];state.statusPrivileged=[];state.statuses=[];directMessenger?.destroy();directMessenger=null;stopPrivateSubscriptions();stopOwnProfileListener();}renderAuth();renderAccount();renderConnections();renderDetailThread();if(next&&state.firebaseReady)ensurePrivateIdentity().catch(e=>{console.error(e);toast('Could not link the private account to a public profile.');});});
    ...
    setBackendStatus('LCS Mobile v0.8 Momentum connected','Public content uses random public profile IDs. Status authorization, soft-delete retention, and immutable moderation logs are enforced by Firestore rules.','ok');

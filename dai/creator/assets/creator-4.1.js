(() => {
  const bundle = window.DAI_CREATOR_41;
  if (!bundle || document.body.dataset.daiWeb41 === '1') return;
  document.body.dataset.daiWeb41 = '1';
  ...
  // Extend the legacy Runtime Definitions picker too, so both Creator paths understand 4.1 folders.
  try {
    if (typeof RUNTIME_DEFINITION_TYPES !== 'undefined') {
      const forceParity=new Set(['reaction_event','data_screen','screen_override','skill']); for (const [id,s] of orderedSchemas()) if (!RUNTIME_DEFINITION_TYPES[id] || forceParity.has(id)) RUNTIME_DEFINITION_TYPES[id]={label:`DAI 4.1 ${s.display_name}`,folder:s.folder,sample:clone(s.template||{})};
  ...
  } catch(e){ console.debug('DAI 4.1 runtime picker extension unavailable',e); }

  const sidebar=$('.sidebar .panel-body');
  if(sidebar && !sidebar.querySelector('[data-view="studio41"]')){
    const label=document.createElement('div'); label.className='section-label datapack-only'; label.textContent='DAI 4.1';
    const btn=document.createElement('button'); btn.className='nav-btn datapack-only'; btn.dataset.view='studio41'; btn.dataset.search='4.1 visual studio scene screen story input creator schema preset modules'; btn.textContent='4.1 Visual Studio';
    const pack=$('.nav-btn[data-view="pack"]'); pack?.after(label,btn);
    btn.addEventListener('click',()=>{ try{switchView('studio41')}catch{document.querySelectorAll('.editor-section').forEach(x=>x.classList.toggle('active',x.id==='view-studio41'));} });
  }

  const dashboardGrid=$('#view-dashboard .start-grid.datapack-only');
  if(dashboardGrid && !dashboardGrid.querySelector('[data-studio41-card]')){
    const card=document.createElement('button'); card.type='button'; card.className='start-card'; card.dataset.studio41Card='1';
    card.innerHTML='<strong>DAI 4.1 Visual Studio</strong><span>Scenes, screens, story archives, input profiles and every schema-defined module.</span>';
    card.addEventListener('click',()=>{ try{switchView('studio41')}catch{} }); dashboardGrid.prepend(card);
  }

  const main=$('.creator-layout main');
  if(main && !$('#view-studio41')){
    const sec=document.createElement('section'); sec.className='editor-section panel datapack-only'; sec.id='view-studio41';
    sec.innerHTML=`<div class="panel-head"><strong>DAI 4.1 Visual Studio</strong><small>Schema-driven authoring for every DAI 4.1 module</small></div><div class="panel-body studio41-body">
      <div class="studio41-hero"><div><span class="system-tag">DAI 4.1 / DATA-DRIVEN</span><h2>Choose what you want to create.</h2><p>The browser Creator now reads the same module concepts as the in-game 4.1 Creator. Pick a category, type and optional preset; Basic shows only the fields needed to begin.</p></div><span class="version-chip">47 authoring types</span></div>
      <div class="studio41-mode"><button class="btn primary" data-s39-level="0">Basic</button><button class="btn" data-s39-level="1">More</button><button class="btn" data-s39-level="2">All</button><a class="btn" href="/dai/guides/version-4-1/" target="_blank">4.1 Guide ↗</a></div>
      <div class="studio41-workspace">
        <aside class="studio41-browser"><div class="studio41-rail"><button data-s39-rail="experience" class="active">GAME</button><button data-s39-rail="standalone">PARTS</button></div><div class="studio41-cats" id="s39Cats"></div><div class="studio41-types" id="s39Types"></div><div class="studio41-presets" id="s39Presets"></div></aside>
        <section class="studio41-preview"><span class="system-tag" id="s39Folder"></span><h3 id="s39Title"></h3><p id="s39Desc"></p><pre id="s39Preview"></pre></section>
        <aside class="studio41-inspector"><div class="field"><label>Definition ID / path</label><input id="s39Id" value="definition"/></div><div class="studio41-fields" id="s39Fields"></div><details><summary>Raw JSON</summary><textarea id="s39Raw" spellcheck="false"></textarea></details><div class="studio41-actions"><button class="btn primary" id="s39Save" type="button">Add / Update Definition</button><button class="btn" id="s39Reset" type="button">Reset</button><button class="btn" id="s39OpenFiles" type="button">Universal Files</button></div><div class="passthrough" id="s39Status">Ready.</div></aside>
      </div></div>`;
  ...
  function renderAll(){ renderCats(); renderTypes(); renderPresets(); sync(); $$('.studio41-rail button').forEach(b=>b.classList.toggle('active',b.dataset.s39Rail===rail)); $$('[data-s39-level]').forEach(b=>b.classList.toggle('primary',Number(b.dataset.s39Level)===level)); }
  $$('.studio41-rail button').forEach(b=>b.onclick=()=>{rail=b.dataset.s39Rail; const first=orderedSchemas().find(([,s])=>s.rail===rail); if(first){category=first[1].category;schemaId=first[0];reset();}renderAll();});

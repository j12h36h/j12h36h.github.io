...
function updateModeUi(){
  ...
  $("#workspaceDescription").textContent = resource
    ? "Create or import a Minecraft resource pack, add textures and other assets, opt into DAI 2.0 companion auto-enable, coordinate directional combat presentation where applicable, then export a normal editable ZIP."
    : "Create a complete DAI Engine 2.0 game or addon: experiences, native entities, worldgen, objectives, server-authoritative actions, menus, reactions and custom content. Universal Files can create anything the guided editor does not expose; the Musashi directional-combat client hook is documented separately.";
...
function validateAction(a,path,issues,objectiveIds){
  ...
  if(!actionMap.has(a.type))issues.push({level:"warn",message:`${path}: unknown/imported action type '${a.type}' is preserved but cannot be fully validated by this DAI 2.0 creator catalog.`});
...
// ---- DAI 2.0 Browser Gameplay Tester ------------------------------------

...
const RUNTIME_DEFINITION_TYPES={
  ...
  sound:{label:"1.9.7 Sound",folder:"dai_sounds",customization:true,carrierResourceId:true,sample:{display_name:"UI Chime",carrier:"minecraft:block.note_block.pling",properties:{source:"master"},numbers:{volume:1.0,pitch:1.0},events:{}}},
  music:{label:"1.9.7 Music",folder:"dai_music",customization:true,carrierResourceId:true,sample:{display_name:"Experience Music",carrier:"minecraft:music.game",numbers:{volume:1.0,pitch:1.0},events:{}}},
  hud:{label:"1.9.7 HUD",folder:"dai_hud",customization:true,sample:{display_name:"Status HUD",texture:"my_dai_pack:gui/status",numbers:{x:0,y:0,width:128,height:32,z:20,ticks:0,alpha:1.0},properties:{anchor:"top_left",color:"#FFFFFF",click_action:""},flags:{interactable:false,consume_click:false,hide_vanilla:false},events:{}}},
  render_profile:{label:"1.9.7 Render Profile",folder:"dai_render_profiles",customization:true,sample:{display_name:"Cinematic Profile",properties:{},numbers:{},flags:{},events:{apply:"action:my_dai_pack:apply_render_profile",clear:"action:my_dai_pack:clear_render_profile"}}},
  structure:{label:"1.9.7 Structure / Natural Structure",folder:"dai_structures",customization:true,carrierResourceId:true,sample:{display_name:"Placed Structure",carrier:"minecraft:village/plains/houses/plains_small_house_1",target:"~ ~ ~",flags:{natural:false},properties:{placement:"surface",dimensions:"minecraft:overworld",rotation:"random",mirror:"none"},numbers:{spacing_chunks:24,separation_chunks:8,frequency:1.0,min_y:-64,max_y:320},events:{}}},
  feature:{label:"1.9.7 World Feature / Natural Feature",folder:"dai_features",customization:true,carrierResourceId:true,sample:{display_name:"Placed Feature",carrier:"minecraft:ore_coal",target:"~ ~ ~",flags:{natural:false},properties:{placement:"surface",dimensions:"minecraft:overworld"},numbers:{spacing_chunks:6,separation_chunks:2,frequency:1.0},events:{}}},
  loot:{label:"1.9.7 Loot",folder:"dai_loot",customization:true,carrierResourceId:true,sample:{display_name:"Reward Loot",carrier:"minecraft:chests/simple_dungeon",events:{}}},
  currency:{label:"1.9.7 Currency",folder:"dai_currencies",customization:true,sample:{display_name:"Credits",properties:{objective:"credits"},numbers:{amount:1.0},events:{}}},
  shop:{label:"1.9.7 Shop",folder:"dai_shops",customization:true,sample:{display_name:"Field Shop",entries:["my_dai_pack:starter_item"],events:{open:"action:my_dai_pack:shop_open",purchase:"action:my_dai_pack:shop_purchase"}}},
  dialogue:{label:"1.9.7 Dialogue",folder:"dai_dialogues",customization:true,sample:{display_name:"NPC Introduction",entries:["Welcome, agent."],events:{start:"action:my_dai_pack:dialogue_start",choose:"action:my_dai_pack:dialogue_choose",end:"action:my_dai_pack:dialogue_end"}}},
  quest:{label:"1.9.7 Quest",folder:"dai_quests",customization:true,sample:{display_name:"First Assignment",entries:["Reach the objective."],events:{start:"action:my_dai_pack:quest_start",advance:"action:my_dai_pack:quest_advance",complete:"action:my_dai_pack:quest_complete",fail:"action:my_dai_pack:quest_fail"}}},
  faction:{label:"1.9.7 Faction",folder:"dai_factions",customization:true,sample:{display_name:"Border",properties:{tag:"faction_border"},events:{}}},
  biome:{label:"1.9.7 Native Biome",folder:"dai_biomes",sample:{has_precipitation:true,temperature:0.8,downfall:0.4,water_color:"#3f76e4",effects:{fog_color:12638463,sky_color:7907327},spawners:{},spawn_costs:{}}},
  dimension_type:{label:"1.9.7 Dimension Type",folder:"dai_dimension_types",sample:{natural:true,ultrawarm:false,has_skylight:true,has_ceiling:false,coordinate_scale:1.0,ambient_light:0.0,min_y:-64,height:384,logical_height:384,infiniburn:"#minecraft:infiniburn_overworld",effects:"minecraft:overworld"}},
  timeline:{label:"1.9.7 Timeline / World Clock",folder:"dai_timelines",sample:{world_clock:"minecraft:overworld",day_length:24000}},
  dimension:{label:"1.9.7 Native Dimension",folder:"dai_dimensions",sample:{generation:{type:"void",dimension_type:"minecraft:overworld",biome:"minecraft:the_void",features:false,lakes:false,structures:false}}},
  native_registry:{label:"1.9.7 Open Mojang Registry Bridge",folder:"dai_registry",sample:{registry:"damage_type",raw:{message_id:"my_damage",exhaustion:0.1,scaling:"never"}}},
  native_data:{label:"1.9.7 Open Server-Data Bridge",folder:"dai_data",sample:{output:"predicate",raw:{condition:"minecraft:entity_properties",entity:"this",predicate:{}}}},
  ruleset:{label:"1.9.7 Ruleset",folder:"dai_rules",customization:true,sample:{display_name:"Adventure Rules",entries:["doDaylightCycle=false","keepInventory=true"],events:{}}},
  vehicle:{label:"1.9.7 Vehicle",folder:"dai_vehicles",customization:true,carrierResourceId:true,sample:{display_name:"Minecart Vehicle",carrier:"minecraft:minecart",target:"~ ~ ~",events:{}}},
  interactive:{label:"1.9.7 Interactive",folder:"dai_interactives",customization:true,sample:{display_name:"Control Console",carrier:"minecraft:lever",events:{use:"action:my_dai_pack:console_use"}}},
  fluid:{label:"1.9.7 Fluid / Environment Block",folder:"dai_fluids",customization:true,carrierResourceId:true,sample:{display_name:"Water Placement",carrier:"minecraft:water",target:"~ ~ ~",events:{}}},
  environment:{label:"1.9.7 Environment",folder:"dai_environments",customization:true,sample:{display_name:"Training Arena",events:{enter:"action:my_dai_pack:arena_enter",exit:"action:my_dai_pack:arena_exit"}}}
};
...
function updateModeUi(){
  ...
  $("#workspaceDescription").textContent = resource
    ? "Create or import a Minecraft resource pack, add textures and other assets, opt into DAI 1.9.7 companion auto-enable with a stable update identity, then export a normal editable ZIP."
    : "Create a complete DAI 1.9.7 game or addon: experiences, native entities, worldgen, objectives, server-authoritative actions, menus, reactions and custom content. Universal Files can create anything the guided editor does not expose.";
...
function renderConditionCard(c,list,index,onChange){
  ...
  if (!logical) {
    ...
    if ((spec?.inputs||[]).includes("target")) dyn += field("target",textInput(c.target,'data-cf="target"'),"Provider input, such as a 1.9.7 customization kind/folder.");
...
const UNIVERSAL_TEMPLATES={
  ...
  native_ai:()=>[`data/${state.pack.namespace}/logics/definitions/native_entity_ai.json`,JSON.stringify({sequence:[{type:"target_player",conditions:[{type:"entity_has_target",operator:"is_false"}]},{type:"look_at_target",conditions:[{type:"entity_has_target",operator:"is_true"}]},{type:"move_to_target",value:1.0,conditions:[{type:"entity_target_distance",operator:"greater_than",number_value:2.0}]},{type:"melee_attack",value:2.0,conditions:[{type:"entity_target_distance",operator:"less_than_or_equal",number_value:2.0},{type:"entity_can_see_target",operator:"is_true"}]},{type:"wander",value:0.8,ticks:5,conditions:[{type:"entity_has_target",operator:"is_false"}]}]},null,2)+"\n"],
...
function generatedFiles(){
  ...
  const packMeta={pack:{description:state.pack.description,min_format:state.pack.minFormat,max_format:state.pack.maxFormat}};if(["main","addon"].includes(state.pack.role))packMeta.dai={role:state.pack.role};files["pack.mcmeta"]=JSON.stringify(packMeta,null,2)+"\n";
...
function referenceEntries(){
  ...
  const conditions=(catalog.conditions||[]).map(x=>({
    kind:"condition",id:x.id,category:x.category||"Uncategorized",purpose:x.purpose||"",
    valueType:x.valueType||"value",
    meta:(x.inputs||[]).length?`inputs: ${(x.inputs||[]).join(", ")}`:"no provider inputs",
    searchable:[x.id,x.category,x.purpose,x.valueType,...(x.inputs||[])].join(" ").toLowerCase()
  }));
...
function renderReferenceLibrary(){
  ...
  root.innerHTML=matches.map(x=>{
    ...
    return `<article class="reference-card">
      <div class="reference-card-head"><span class="reference-kind ${x.kind}">${x.kind}</span>${operatorHint}<code>${esc(x.id)}</code></div>
...
function validateAction(a,path,issues,objectiveIds){
  ...
  if(!actionMap.has(a.type))issues.push({level:"warn",message:`${path}: unknown/imported action type '${a.type}' is preserved but cannot be fully validated by this DAI 1.9.7 creator catalog.`});
...
// ---- DAI 1.9.7 Browser Gameplay Tester ------------------------------------

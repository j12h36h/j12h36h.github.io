...
function updateModeUi(){
  ...
  $("#workspaceDescription").textContent = resource
    ? "Create or import a Minecraft resource pack, add textures and other assets, opt into DAI 1.9.9 companion auto-enable with a stable update identity, then export a normal editable ZIP."
    : "Create a complete DAI 1.9.9 game or addon: experiences, native entities, worldgen, objectives, server-authoritative actions, menus, reactions and custom content. Universal Files can create anything the guided editor does not expose.";
...
function validateAction(a,path,issues,objectiveIds){
  ...
  if(!actionMap.has(a.type))issues.push({level:"warn",message:`${path}: unknown/imported action type '${a.type}' is preserved but cannot be fully validated by this DAI 1.9.9 creator catalog.`});
...
// ---- DAI 1.9.9 Browser Gameplay Tester ------------------------------------

...
const safeTint = value => /^#[0-9a-fA-F]{6}$/.test(String(value||'')) ? String(value).toLowerCase() : '#ffffff';
const state = { identity:null, catalog:{assets:[]}, asset:null, inventoryUnsub:null, creditUnsub:null, creditBalance:0, image:null, holdings:[], archiveView:false };
...
$('#assetTint')?.addEventListener('input',event=>{renderTint(event.target.value,$('#assetCanvas'));updateObtainState();});
document.querySelectorAll('[data-tint]').forEach(button=>button.addEventListener('click',()=>{$('#assetTint').value=button.dataset.tint;renderTint(button.dataset.tint,$('#assetCanvas'));updateObtainState();}));

...
function projectManagementMarkup(o){
  ...
  return `<section class="project-access-section"><div class="section-heading compact-heading"><div><p class="eyebrow">Project people & Status</p><h3>Project-specific access</h3></div></div><p class="muted">The project creator is always an implicit Project manager. Project Moderators can manage this project without becoming global moderators.</p><div class="project-member-list">${list}</div>${canManage?`<div class="project-access-controls" data-project-id="${escapeHtml(o.id)}"><label class="field"><span>Person</span><select data-project-person>${options||'<option value="">No other public profiles available</option>'}</select></label><div class="project-access-grid"><label class="field"><span>Status</span><select data-project-status><option value="moderator">Project Moderator</option><option value="timeout">Project Timeout</option></select></label><label class="field"><span>Duration</span><select data-project-duration><option value="none">No expiration</option><option value="hour">1 hour</option><option value="day">24 hours</option><option value="week">7 days</option></select></label></div><label class="field"><span>Reason / note</span><input data-project-reason maxlength="240" placeholder="Optional note; used for Timeout audit context"></label><button class="primary-button" data-project-grant type="button">Grant project Status</button></div><details class="project-edit-panel"><summary>Edit project details</summary><div class="project-edit-fields" data-project-edit="${escapeHtml(o.id)}"><label class="field"><span>Project name</span><input data-project-title maxlength="100" value="${escapeHtml(o.title)}"></label><label class="field"><span>Description</span><textarea data-project-description maxlength="700">${escapeHtml(o.description)}</textarea></label><label class="field"><span>Tags</span><input data-project-tags maxlength="120" value="${escapeHtml((o.tags||[]).join(', '))}"></label><button class="primary-button" data-project-save type="button">Save project</button></div></details>`:''}</section>`;
}
...
async function revokeStatus(id){
  ...
  const allowed=isFounder()||(row.status==='timeout'&&canModerateStatusScope(row.scopeType,row.scopeId))||(row.status==='moderator'&&row.scopeType==='project'&&canModerateStatusScope(row.scopeType,row.scopeId));if(!allowed){toast('You do not have authority to remove this Status.');return;}

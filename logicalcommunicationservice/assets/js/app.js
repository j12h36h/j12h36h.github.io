...
async function grantStatus(e){
  ...
  }catch(error){console.error('grant status failed',error);const msg=error?.code==='permission-denied'&&isFounder()?'Firestore did not recognize this profile as the configured root Founder. Verify systemPrivate/founder.profileId matches your current LCS public profile ID, then publish the v0.9.3 rules.':firestoreErrorText(error,'grant this Status');toast(msg);}
...
async function initFirebase(){
  ...
  try{
    ...
    setBackendStatus('LCS v0.9.3 moderation authority + Activity notifications connected','Public content uses random public profile IDs. Status authorization, soft-delete retention, and immutable moderation logs are enforced by Firestore rules.','ok');
  }catch(e){console.error(e);state.authReady=true;setBackendStatus('Could not connect to Firebase','Check Firebase configuration and publish the included v0.9.3 firestore.rules.','error');renderAuth();renderAccount();}
}

...
function canAccessModerationClient(profileId=state.profileId){return Boolean(profileId&&(isFounder(profileId)||activeStatusesFor(profileId).some(x=>x.status==='moderator')));}
...
async function revokeStatus(id){
  ...
  const allowed=isFounder()||(row.status==='timeout'&&canModerateStatusScope(row.scopeType,row.scopeId));if(!allowed){toast('You do not have authority to remove this Status.');return;}
...
async function initFirebase(){
  ...
  try{
    ...
    publicSubscribe('publicObjects',rows=>{state.objects=rows;synthesizeReferencedProfiles();renderCatalogs();renderUniverse();renderTrends();renderSearchPanel();renderConnections();renderStatusTargetOptions();renderActivity();hydrateReferencedProfiles().catch(console.debug);},{orderBy:'createdAt',limit:350,filters:[['deleted','==',false]]});

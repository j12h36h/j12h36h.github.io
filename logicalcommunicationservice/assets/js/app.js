...
function showAuthError(err){const code=String(err?.code||'auth/unknown');$('#authErrorTitle').textContent='Google sign-in needs attention';$('#authErrorText').textContent=code.includes('unauthorized-domain')?'This domain is not authorized in Firebase Authentication.':code.includes('popup-closed')?'The Google window was closed before sign-in finished.':code.includes('popup-blocked')?'The browser blocked the Google popup. Allow popups for this site and retry.':code.includes('internal-error')?'Firebase returned an internal sign-in error. On mobile, LCS now prepares Firebase persistence during startup so the Google popup can open directly from your tap without an awaited operation consuming the browser's user-activation window. If this still appears, open the site in the device's full browser rather than an in-app browser and confirm the authorized domains/browser-key origins.':'Firebase could not complete Google sign-in. Check Authentication, authorized domains, and browser key restrictions.';$('#authErrorCode').textContent=code;$('#authErrorBox').hidden=false;}
async function signInGoogle(){clearAuthError();if(!state.firebase?.auth)return showAuthError({code:'auth/configuration-not-found'});const {auth,authMod}=state.firebase;const provider=new authMod.GoogleAuthProvider();provider.setCustomParameters({prompt:'select_account'});try{await authMod.signInWithPopup(auth,provider);closeDialog('#authDialog');toast('Signed in. LCS is linking a separate public identity.');}catch(e){console.error(e);showAuthError(e);}}
...
async function initFirebase(){
  ...
  try{
    ...
    const app=appMod.initializeApp(LCS_CONFIG.firebase),auth=authMod.getAuth(app);authMod.useDeviceLanguage(auth);await authMod.setPersistence(auth,authMod.browserLocalPersistence);state.firebase={app,auth,authMod,db:null,fsMod:null};
    ...
    setBackendStatus('LCS v0.7.7 mobile auth + popup + discussion sync connected','Public content uses random public profile IDs. Status authorization, soft-delete retention, and immutable moderation logs are enforced by Firestore rules.','ok');

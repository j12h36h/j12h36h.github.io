...
const STATUS_META = Object.freeze({
  ...
  member: { label: 'Member', symbol: '●', public: true, description: 'Project membership or a project-defined participant role; grants no moderation authority.' },
...
function referencedProfileIds() {
  ...
  state.friendRequests.forEach(x=>{add(x.fromProfileId);add(x.toProfileId);}); state.friendships.forEach(x=>(x.members||[]).forEach(add)); state.communityMembers.forEach(x=>add(x.profileId));
  return [...ids];
}
...
function renderPublicProfileDirectory(){
  ...
  root.innerHTML=rows.length?rows.map(p=>renderPersonTradeRow(p.id,p._stub?'Profile metadata pending sync':(p.bio||'Public LCS profile'))).join(''):'<p class="muted">No other public profiles have synced yet.</p>';
}
...
function renderMomentumDeck(){
  ...
  root.innerHTML=entries.map(({type,item})=>{
    if(type==='lfg'){const who=identity(item.authorProfileId);return `<article class="momentum-card momentum-lfg"><button class="momentum-card-open" data-open-lfg="${escapeHtml(item.id)}" type="button" aria-label="Open LFG: ${escapeHtml(item.title)}"><div class="momentum-card-top"><span>↔ COLLABORATE</span><small>${timeAgo(item.createdAt)}</small></div><h3>${escapeHtml(item.title)}</h3><p>${escapeHtml(item.description)}</p></button><div class="momentum-card-foot"><span>${escapeHtml(who.displayName)} · ${escapeHtml(item.topic)}</span><button data-open-lfg="${escapeHtml(item.id)}" type="button">Open match →</button></div></article>`;}
    if(type==='post'){const p=item,r=reasoning[p.reasoningType]||reasoning.unclassified;return `<article class="momentum-card"><button class="momentum-card-open" data-open-post="${escapeHtml(p.id)}" type="button" aria-label="Open post"><div class="momentum-card-top"><span>${r.symbol} ${escapeHtml(r.plain)}</span><small>${timeAgo(p.createdAt)}</small></div><p class="momentum-post-copy">${escapeHtml(p.text)}</p></button><div class="momentum-card-actions">${momentumActionsForPost(p)}</div></article>`;}
    const o=item;return `<article class="momentum-card"><button class="momentum-card-open" data-open-object="${escapeHtml(o.id)}" type="button" aria-label="Open ${escapeHtml(o.kind)}: ${escapeHtml(o.title)}"><div class="momentum-card-top"><span>${o.kind==='idea'?'💡 IDEA':o.kind==='problem'?'? PROBLEM':'◆ PROJECT'}</span><small>${timeAgo(o.createdAt)}</small></div><h3>${escapeHtml(o.title)}</h3><p>${escapeHtml(o.description)}</p>${momentumMarkup(o,true)}</button><div class="momentum-card-actions">${momentumActionsForObject(o)}</div></article>`;
  }).join('');
}
...
async function verifyFounderAuthority(){
  if(!state.profileId||!state.firebaseReady)return false;
  if(state.founderAuthorityVerified)return true;
  if(state.founderAuthorityCheckPromise)return state.founderAuthorityCheckPromise;
  const {db,fsMod}=state.firebase;
  state.founderAuthorityCheckPromise=(async()=>{
    try{
      // v0.9.3 rules expose no data here; they only authorize this empty-document get for the configured root Founder.
      await fsMod.getDoc(fsMod.doc(db,'systemAuthority','founderProbe'));
      state.founderAuthorityVerified=true;
      await ensureCanonicalFounderStatus();
      renderStatusSurfaces();renderAuth();renderAccount();
      return true;
    }catch(e){
      state.founderAuthorityVerified=false;
      if(e?.code!=='permission-denied'&&e?.code!=='not-found')console.debug('Founder authority probe',e?.code||e);
      renderStatusSurfaces();
      return false;
    }finally{state.founderAuthorityCheckPromise=null;}
  })();
  return state.founderAuthorityCheckPromise;
}

function activityStorageKey(){return state.profileId?`lcsActivitySeen:${state.profileId}`:'lcsActivitySeen';}
function loadActivitySeen(){if(!state.profileId){state.activitySeenAt=0;return;}try{const n=Number(localStorage.getItem(activityStorageKey())||0);state.activitySeenAt=Number.isFinite(n)?n:0;}catch{state.activitySeenAt=0;}}
function markActivitySeen(){if(!state.profileId)return;state.activitySeenAt=Date.now();try{localStorage.setItem(activityStorageKey(),String(state.activitySeenAt));}catch{}updateActivityBadge();}
function activityTargetLabel(type,id){if(type==='post'){const x=state.posts.find(p=>p.id===id)||state.moderationPosts.find(p=>p.id===id);return x?String(x.text||'your post').slice(0,90):'your post';}if(type==='object'){const x=state.objects.find(o=>o.id===id)||state.moderationObjects.find(o=>o.id===id);return x?.title||'your item';}if(type==='lfg'){return state.lfg.find(x=>x.id===id)?.title||'your LFG listing';}return 'your activity';}
function buildActivityItems(){
  if(!state.profileId)return [];
  const pid=state.profileId, ownPosts=new Set(state.posts.filter(x=>x.authorProfileId===pid).map(x=>x.id)), ownObjects=new Set(state.objects.filter(x=>x.authorProfileId===pid).map(x=>x.id)), ownLfg=new Set(state.lfg.filter(x=>x.authorProfileId===pid).map(x=>x.id));
  const items=[];
  state.comments.forEach(c=>{if(c.authorProfileId===pid||c.deleted)return;const mine=c.targetType==='post'?ownPosts.has(c.targetId):ownObjects.has(c.targetId);if(!mine)return;items.push({id:`comment:${c.id}`,kind:'comment',at:c.createdAt,actor:c.authorProfileId,title:`${identity(c.authorProfileId).displayName} responded to ${activityTargetLabel(c.targetType,c.targetId)}`,detail:String(c.text||'').slice(0,180),openType:c.targetType,openId:c.targetId});});
  state.reactions.forEach(r=>{if(r.actorProfileId===pid)return;const mine=r.targetType==='post'?ownPosts.has(r.targetId):ownObjects.has(r.targetId);if(!mine)return;items.push({id:`reaction:${r.id}`,kind:'reaction',at:r.createdAt,actor:r.actorProfileId,title:`${identity(r.actorProfileId).displayName} marked ${activityTargetLabel(r.targetType,r.targetId)} helpful`,detail:'Helpful reaction',openType:r.targetType,openId:r.targetId});});
  state.follows.forEach(f=>{if(f.followerProfileId===pid)return;const mine=f.targetType==='profile'?f.targetId===pid:(f.targetType==='object'&&ownObjects.has(f.targetId));if(!mine)return;items.push({id:`follow:${f.id}`,kind:'follow',at:f.createdAt,actor:f.followerProfileId,title:f.targetType==='profile'?`${identity(f.followerProfileId).displayName} followed your profile`:`${identity(f.followerProfileId).displayName} followed ${activityTargetLabel('object',f.targetId)}`,detail:'New follow',openType:f.targetType==='profile'?'profile':'object',openId:f.targetType==='profile'?f.followerProfileId:f.targetId});});
  state.friendRequests.filter(x=>x.toProfileId===pid&&x.fromProfileId!==pid).forEach(x=>items.push({id:`friend:${x.id}`,kind:'friend',at:x.updatedAt||x.createdAt,actor:x.fromProfileId,title:`${identity(x.fromProfileId).displayName} sent a friend request`,detail:x.status==='pending'?'Waiting for your response':`Request ${x.status}`,openType:'connections',openId:''}));
  state.lfgRequests.filter(x=>x.toProfileId===pid&&x.fromProfileId!==pid).forEach(x=>items.push({id:`lfg:${x.id}`,kind:'lfg',at:x.updatedAt||x.createdAt,actor:x.fromProfileId,title:`${identity(x.fromProfileId).displayName} responded to ${activityTargetLabel('lfg',x.lfgId)}`,detail:x.status==='pending'?'LFG request waiting for your response':`Request ${x.status}`,openType:'connections',openId:''}));
  state.statusOwn.forEach(x=>{const at=x.updatedAt||x.createdAt;if(!at)return;const label=STATUS_META[x.status]?.label||x.status;items.push({id:`status:${x.id}:${timeValue(at)}`,kind:'status',at,actor:x.grantedByProfileId,title:x.active?`${label} Status is active on your account`:`${label} Status was removed from your account`,detail:statusScopeDescription(x),openType:'account',openId:''});});
  return [...new Map(items.map(x=>[x.id,x])).values()].sort((a,b)=>timeValue(b.at)-timeValue(a.at));
}
function updateActivityBadge(){const badge=$('#activityUnreadBadge');if(!badge)return;const count=buildActivityItems().filter(x=>timeValue(x.at)>state.activitySeenAt).length;badge.textContent=String(Math.min(count,99));badge.hidden=!state.profileId||count===0;badge.setAttribute('aria-label',`${count} unread notifications`);const summary=$('#activityUnreadCount');if(summary)summary.textContent=count?`${count} unread`:'Caught up';}
function activityIcon(kind){return ({comment:'↩',reaction:'♡',follow:'★',friend:'↔',lfg:'⚑',status:'◆'})[kind]||'•';}
function renderActivity(){
  const signedOut=$('#activitySignedOut'),signedIn=$('#activitySignedIn'),list=$('#activityList');if(!signedOut||!signedIn||!list)return;
  const ready=Boolean(state.profileId);signedOut.hidden=ready;signedIn.hidden=!ready;if(!ready){updateActivityBadge();return;}
  const rows=buildActivityItems();const unread=rows.filter(x=>timeValue(x.at)>state.activitySeenAt).length;const summary=$('#activityUnreadCount');if(summary)summary.textContent=unread?`${unread} unread`:'Caught up';
  list.innerHTML=rows.length?rows.map(x=>`<button class="activity-row ${timeValue(x.at)>state.activitySeenAt?'activity-unread':''}" data-activity-open="${escapeHtml(x.openType)}" data-activity-id="${escapeHtml(x.openId||'')}" type="button"><span class="activity-icon">${activityIcon(x.kind)}</span><span class="activity-copy"><b>${escapeHtml(x.title)}</b><small>${escapeHtml(x.detail||'')}</small></span><time>${timeAgo(x.at)}</time></button>`).join(''):'<div class="activity-empty"><b>No activity yet.</b><span>Comments, helpful reactions, follows, requests, and Status changes involving your work will appear here.</span></div>';
  updateActivityBadge();
}
function openActivityItem(type,id){if(type==='post')return openPostDetail(id);if(type==='object')return openObjectDetail(id);if(type==='profile')return openProfileDetail(id);if(type==='connections')return setView('connections');if(type==='account')return setView('account');}
...
function communityMemberId(spaceId, profileId) { return safeDocId(spaceId, profileId); }
function communityMembersFor(spaceId) { return state.communityMembers.filter(m => m.spaceId === spaceId); }
function ownCommunityMembership(spaceId) { return communityMembersFor(spaceId).find(m => m.profileId === state.profileId) || null; }
async function ensureOwnedCommunityMemberships() {
  if(!state.firebaseReady||!state.profileId)return;const {db,fsMod}=state.firebase;
  const missing=state.spaces.filter(s=>!s.system&&s.ownerProfileId===state.profileId&&!communityMembersFor(s.id).some(m=>m.profileId===state.profileId));
  for(const space of missing.slice(0,50)){try{await fsMod.setDoc(fsMod.doc(db,'publicCommunityMembers',communityMemberId(space.id,state.profileId)),{spaceId:space.id,profileId:state.profileId,role:'owner',joinedAt:fsMod.serverTimestamp(),updatedAt:fsMod.serverTimestamp()},{merge:false});}catch(error){if(error?.code!=='permission-denied')console.debug('Community owner membership backfill',space.id,error?.code||error);}}
}
function communityRole(spaceId, profileId) { return communityMembersFor(spaceId).find(m => m.profileId === profileId)?.role || ''; }
function canModerateCommunityClient(space) {
  if (!space || space.system || !state.profileId) return false;
  if (space.ownerProfileId === state.profileId || isFounder()) return true;
  return communityRole(space.id, state.profileId) === 'moderator';
}
function canEditCommunityClient(space) { return Boolean(space && !space.system && state.profileId && (space.ownerProfileId === state.profileId || isFounder())); }
function participantMarkup(space, canEdit) {
  const members = communityMembersFor(space.id).slice().sort((a,b) => {
    const rank = { owner: 0, moderator: 1, member: 2 };
    return (rank[a.role] ?? 9) - (rank[b.role] ?? 9) || identity(a.profileId).displayName.localeCompare(identity(b.profileId).displayName);
  });
  if (!members.length) return '<p class="muted">No participant records yet.</p>';
  return `<div class="community-participant-list">${members.slice(0,200).map(member => {
    const who = identity(member.profileId), role = member.role || 'member';
    const canManageRole = canEdit && role !== 'owner' && member.profileId !== state.profileId;
    const roleAction = role === 'moderator' ? 'member' : 'moderator';
    return `<div class="community-participant-row"><button class="identity-button" data-open-profile="${escapeHtml(member.profileId)}" type="button">${avatarMarkup(who)}<span><b>${escapeHtml(who.displayName)}</b><small>${escapeHtml(role.toUpperCase())}</small></span></button><div class="community-participant-actions">${canManageRole?`<button class="ghost-button" data-community-role="${escapeHtml(space.id)}" data-profile-id="${escapeHtml(member.profileId)}" data-role="${roleAction}" type="button">${role==='moderator'?'Remove moderator':'Make moderator'}</button><button class="ghost-button danger-button" data-community-remove-participant="${escapeHtml(space.id)}" data-profile-id="${escapeHtml(member.profileId)}" type="button">Remove</button>`:''}</div></div>`;
  }).join('')}</div>`;
}
async function ensureWritableChannel(channel) {
  ...
  if(!canModerateCommunityClient(space))throw new Error('This community still uses a legacy #general channel. A community moderator must repair it before new posts can be published there.');
...
function setView(view, updateHash = true) {
  ...
  if(requested==='moderation'&&!canAccessModerationClient()){
    requested='home';
    if(state.authReady&&state.profileId&&updateHash)toast('Moderator or root Founder Status is required for the moderation workspace.');
  }
  ...
  if (state.activeView === 'activity') { markActivitySeen(); renderActivity(); }
  ...
  renderMomentumDeck(); renderSessionMomentum();
}
...
function renderChannelSelects() {
  ...
  const owned = state.spaces.filter(s => canModerateCommunityClient(s));
...
function renderFeed() {
  ...
  root.innerHTML = items.slice(0,160).map(p => { const who = identity(p.authorProfileId); const r = reasoning[p.reasoningType] || reasoning.unclassified; const helpful = isOwnReaction('post',p.id); const channel = channelById(itemChannelId(p),p.spaceId); const space=spaceById(p.spaceId); return `<article class="post-card"><div class="post-head"><div class="post-head-context"><button class="post-author identity-button" data-open-profile="${escapeHtml(p.authorProfileId)}" type="button">${avatarMarkup(who)}<span class="post-author-copy"><b>${escapeHtml(who.displayName)}</b><small>#${escapeHtml(channel.name)} · ${timeAgo(p.createdAt)}</small></span></button><button class="feed-context-button" data-open-community="${escapeHtml(space.id)}" type="button">${escapeHtml(space.name)}</button></div><button class="reason-pill" data-reason="${escapeHtml(p.reasoningType)}" type="button"><span>${r.symbol}</span>${escapeHtml(r.plain)}</button></div><button class="post-body-button" data-open-post="${escapeHtml(p.id)}" type="button"><p class="post-text">${escapeHtml(p.text)}</p>${(p.tags||[]).length?`<div class="post-tag-row tag-row">${p.tags.map(t=>`<span>${escapeHtml(t)}</span>`).join('')}</div>`:''}</button><div class="post-actions"><button class="ghost-button" data-open-post="${escapeHtml(p.id)}" type="button">💬 Responses · ${responseCount('post',p.id)}</button><button class="ghost-button ${helpful ? 'active-action' : ''}" data-helpful-type="post" data-helpful-id="${escapeHtml(p.id)}" type="button">${helpful ? '♥' : '♡'} Helpful · ${reactionCount('post',p.id)}</button><button class="ghost-button" data-connect-post="${escapeHtml(p.id)}" type="button">↗ Connect</button></div></article>`; }).join('');
}
...
function canManageCommunityClient(space){return canModerateCommunityClient(space);}
function renderCommunities() {
  const root=$('#communityCatalog'); if(!root)return;
  root.innerHTML=allSpaces().map(s=>{
    const cs=channelsForSpace(s.id), count=state.posts.filter(p=>p.spaceId===s.id&&!p.deleted).length+state.objects.filter(o=>o.spaceId===s.id&&!o.deleted).length;
    const participants=s.system?0:communityMembersFor(s.id).length, canManage=canModerateCommunityClient(s);
    return `<article class="community-card"><div class="community-card-head"><button class="community-profile-button" data-open-community="${escapeHtml(s.id)}" type="button"><p class="eyebrow">${s.system?'Network commons':'Public community'}</p><h2>${escapeHtml(s.name)}</h2><p>${escapeHtml(s.description||'No description yet.')}</p></button><div class="community-stats"><span><b>${cs.length}</b> channels</span><span><b>${participants}</b> participants</span><span><b>${count}</b> items</span></div></div><div class="community-channel-grid">${cs.map(c=>`<button class="community-channel-card" data-channel-filter="${escapeHtml(c.id)}" type="button"><span class="channel-symbol">${channelMeta(c.type).symbol}</span><span><b># ${escapeHtml(c.name)}</b><small>${escapeHtml(c.description||channelMeta(c.type).label)}</small></span><em>${state.posts.filter(p=>itemChannelId(p)===c.id&&!p.deleted).length}</em></button>`).join('')}</div>${canManage?`<div class="community-card-actions"><button class="ghost-button" data-open-community="${escapeHtml(s.id)}" type="button">⚙ Manage community</button><button class="ghost-button" data-new-channel="${escapeHtml(s.id)}" type="button">＋ Add channel</button></div>`:''}</article>`;
  }).join('');
}

function ensureCommunityDialog(){
  let dialog=$('#communityDetailDialog'); if(dialog)return dialog;
  dialog=document.createElement('dialog');dialog.className='modal detail-modal';dialog.id='communityDetailDialog';dialog.innerHTML=`<button aria-label="Close" class="modal-close" data-close-dialog type="button">×</button><p class="eyebrow">Community profile</p><h2 id="communityDetailTitle">Community</h2><div id="communityDetailBody"></div>`;document.body.appendChild(dialog);return dialog;
}
function channelManagementMarkup(space, channels, canModerate){
  if(!canModerate)return '';
  const editable=channels.filter(c=>!c.system&&!c.virtual);
  return `<section class="community-manager-panel"><div class="community-manager-heading"><div><p class="eyebrow">Channel management</p><h3>Customize channels</h3></div><button class="ghost-button" data-new-channel="${escapeHtml(space.id)}" type="button">＋ Add channel</button></div>${editable.length?`<div class="community-channel-manager-list">${editable.map(c=>`<form class="community-channel-edit" data-channel-edit="${escapeHtml(c.id)}"><div class="community-channel-edit-grid"><label class="field"><span>Name</span><input data-channel-name maxlength="40" minlength="2" value="${escapeHtml(c.name)}" required></label><label class="field"><span>Type</span><select data-channel-type>${Object.entries(CHANNEL_TYPES).map(([id,meta])=>`<option value="${escapeHtml(id)}" ${c.type===id?'selected':''}>${escapeHtml(meta.label)}</option>`).join('')}</select></label></div><label class="field"><span>Description</span><input data-channel-description maxlength="240" value="${escapeHtml(c.description||'')}"></label><div class="modal-actions"><button class="ghost-button danger-button" data-channel-delete="${escapeHtml(c.id)}" type="button">Delete channel</button><button class="primary-button" type="submit">Save channel</button></div></form>`).join('')}</div>`:'<p class="muted">No editable channels yet.</p>'}</section>`;
}
function openCommunityDetail(id){
  const space=spaceById(id);if(!space)return;const dialog=ensureCommunityDialog(),body=$('#communityDetailBody',dialog),canModerate=canModerateCommunityClient(space),canEdit=canEditCommunityClient(space),channels=channelsForSpace(space.id),owner=space.system?null:identity(space.ownerProfileId);
  $('#communityDetailTitle',dialog).textContent=space.name;
  const items=state.posts.filter(p=>p.spaceId===space.id&&!p.deleted).length+state.objects.filter(o=>o.spaceId===space.id&&!o.deleted).length;
  const members=space.system?[]:communityMembersFor(space.id), own=space.system?null:ownCommunityMembership(space.id), joined=Boolean(own), participantCount=members.length;
  const joinControls=space.system?'':(space.ownerProfileId===state.profileId?'':state.profileId?(joined?`<button class="ghost-button" data-community-leave="${escapeHtml(space.id)}" type="button">Leave community</button>`:`<button class="primary-button" data-community-join="${escapeHtml(space.id)}" type="button">Join community</button>`):`<button class="primary-button" data-open-auth type="button">Sign in to join</button>`);
  body.innerHTML=`<section class="community-profile-summary"><p>${escapeHtml(space.description||'No description yet.')}</p><div class="community-stats"><span><b>${channels.length}</b> channels</span><span><b>${participantCount}</b> participants</span><span><b>${items}</b> public items</span>${owner?`<span>Created by <b>${escapeHtml(owner.displayName)}</b></span>`:''}</div><div class="modal-actions">${joinControls}</div></section><div class="community-channel-grid">${channels.map(c=>`<button class="community-channel-card" data-channel-filter="${escapeHtml(c.id)}" type="button"><span class="channel-symbol">${channelMeta(c.type).symbol}</span><span><b># ${escapeHtml(c.name)}</b><small>${escapeHtml(c.description||channelMeta(c.type).label)}</small></span><em>${state.posts.filter(p=>itemChannelId(p)===c.id&&!p.deleted).length}</em></button>`).join('')}</div><section class="community-participants-panel"><div class="community-manager-heading"><div><p class="eyebrow">Participants</p><h3>${participantCount} participant${participantCount===1?'':'s'}</h3></div></div>${participantMarkup(space,canEdit)}</section>${canEdit?`<section class="community-manager-panel"><p class="eyebrow">Community identity</p><form id="communityEditForm" data-community-id="${escapeHtml(space.id)}"><label class="field"><span>Name</span><input id="communityEditName" maxlength="50" minlength="2" value="${escapeHtml(space.name)}" required></label><label class="field"><span>Description</span><textarea id="communityEditDescription" maxlength="240">${escapeHtml(space.description||'')}</textarea></label><div class="modal-actions"><button class="primary-button" type="submit">Save community</button></div></form></section>`:''}${channelManagementMarkup(space,channels,canModerate)}`;
  showDialog(dialog);
}
async function saveCommunityEdit(e){
  e.preventDefault();if(!requireContribution())return;const form=e.currentTarget,id=form.dataset.communityId,space=state.spaces.find(x=>x.id===id);if(!space||!canEditCommunityClient(space))return toast('You do not have permission to edit this community.');
  const name=$('#communityEditName',form)?.value.trim().replace(/\s+/g,' ')||'',description=$('#communityEditDescription',form)?.value.trim()||'';if(name.length<2)return toast('Community name must be at least 2 characters.');
  const button=form.querySelector('button[type="submit"]'),old=button?.textContent||'Save community';if(button){button.disabled=true;button.textContent='Saving…';}
  try{const {db,fsMod}=state.firebase;await fsMod.updateDoc(fsMod.doc(db,'publicSpaces',id),{name:name.slice(0,50),description:description.slice(0,240),updatedAt:fsMod.serverTimestamp()});state.spaces=state.spaces.map(x=>x.id===id?{...x,name:name.slice(0,50),description:description.slice(0,240),updatedAt:Date.now()}:x);renderSpaces();renderCommunities();renderFeed();renderCatalogs();openCommunityDetail(id);toast('Community updated.');}catch(error){console.error('community edit failed',error);toast(firestoreErrorText(error,'edit this community'));}finally{if(button){button.disabled=false;button.textContent=old;}}
}
...
async function leaveCommunity(spaceId){
  if(!requireUser())return;const membership=ownCommunityMembership(spaceId),space=state.spaces.find(s=>s.id===spaceId);if(!membership||!space)return;if(membership.role==='owner'||space.ownerProfileId===state.profileId)return toast('The community owner cannot leave their own community.');
  try{const {db,fsMod}=state.firebase;await fsMod.deleteDoc(fsMod.doc(db,'publicCommunityMembers',membership.id));toast(`Left ${space.name}.`);}catch(error){console.error('community leave failed',error);toast(firestoreErrorText(error,'leave this community'));}
}
...
function renderSearchPanel() {
  ...
  const groups=[];
  if(profiles.length) groups.push(`<div class="search-group"><b>People</b>${profiles.map(p=>`<div class="search-profile-result"><button data-open-profile="${escapeHtml(p.id)}" type="button"><span>${escapeHtml(p.displayName)}</span><small>${escapeHtml(p.bio||'Public LCS profile')}</small></button>${profileTradeActions(p.id,'search')}</div>`).join('')}</div>`);
...
function profileTradeActions(profileId, context='row') {
  ...
  const messageAction=friendshipStateWith(profileId).kind==='friends'?`<button class="profile-credit-link" data-message-profile="${escapeHtml(profileId)}" type="button" title="Chat" aria-label="Chat with ${escapeHtml(identity(profileId).displayName)}">💬 <span>CHAT</span></button>`:'';
...
function renderConnections() {
  ...
  const friends=[...new Set(friendProfileIds())]; $('#friendCountBadge').textContent=String(friends.length); $('#friendList').innerHTML=friends.length?friends.map(id=>renderPersonTradeRow(id,'Friend')).join(''):'<p class="muted">No accepted friends yet.</p>';
...
async function openProfileDetail(id) {
  ...
  const tradeButton=!self&&!blocked&&signed?profileTradeActions(id,'detail'):'';
...
function projectStatusRows(projectId){return state.statuses.filter(x=>x.scopeType==='project'&&x.scopeId===projectId&&isStatusActive(x));}
function projectManagementMarkup(o){
  if(!o||o.kind!=='project')return '';
  const canManage=canManageProjectClient(o),owner=identity(o.authorProfileId),rows=projectStatusRows(o.id),people=new Map();people.set(o.authorProfileId,{profileId:o.authorProfileId,owner:true,statuses:[]});rows.forEach(r=>{const p=people.get(r.profileId)||{profileId:r.profileId,owner:false,statuses:[]};p.statuses.push(r);people.set(r.profileId,p);});
  const list=[...people.values()].map(entry=>{const p=identity(entry.profileId);return `<div class="project-member-row"><div class="project-member-identity"><button class="identity-link" data-open-profile="${escapeHtml(entry.profileId)}" type="button">${escapeHtml(p.displayName)}</button><small>${entry.owner?'Creator · implicit Project manager':escapeHtml(publicIdShort(entry.profileId))}</small><div class="status-badge-row">${entry.owner?'<span class="status-badge status-moderator">◆ Project owner</span>':''}${entry.statuses.map(x=>`<span class="status-badge status-${escapeHtml(x.status)}">${STATUS_META[x.status]?.symbol||'•'} ${escapeHtml(STATUS_META[x.status]?.label||x.status)}${x.status==='member'&&x.reason?` · ${escapeHtml(x.reason)}`:''}</span>`).join('')}</div></div><div class="request-actions">${canManage?entry.statuses.map(x=>`<button class="ghost-button danger-button" data-status-revoke="${escapeHtml(x.id)}" type="button">Remove ${escapeHtml(STATUS_META[x.status]?.label||x.status)}</button>`).join(''):''}</div></div>`;}).join('');
  const options=Object.values(state.profiles).filter(p=>p.id!==o.authorProfileId&&!p._stub).sort((a,b)=>String(a.displayName||'').localeCompare(String(b.displayName||''))).map(p=>`<option value="${escapeHtml(p.id)}">${escapeHtml(p.displayName||'Member')} · ${escapeHtml(publicIdShort(p.id))}</option>`).join('');
  return `<section class="project-access-section"><div class="section-heading compact-heading"><div><p class="eyebrow">Project people & Status</p><h3>Project-specific access</h3></div></div><p class="muted">The project creator is always an implicit Project manager. Project Moderators can manage the project; Project Members identify participants without granting moderation authority. Member notes can describe a project-defined role such as Player, Builder, Tester, Writer, or anything else the project uses.</p><div class="project-member-list">${list}</div>${canManage?`<div class="project-access-controls" data-project-id="${escapeHtml(o.id)}"><label class="field"><span>Person</span><select data-project-person>${options||'<option value="">No other public profiles available</option>'}</select></label><div class="project-access-grid"><label class="field"><span>Status</span><select data-project-status><option value="member">Project Member</option><option value="moderator">Project Moderator</option><option value="timeout">Project Timeout</option></select></label><label class="field"><span>Duration</span><select data-project-duration><option value="none">No expiration</option><option value="hour">1 hour</option><option value="day">24 hours</option><option value="week">7 days</option></select></label></div><label class="field"><span>Reason / note</span><input data-project-reason maxlength="240" placeholder="Member role/note (Player, Builder, Tester, etc.) or Timeout reason"></label><button class="primary-button" data-project-grant type="button">Grant project Status</button></div><details class="project-edit-panel"><summary>Edit project details</summary><div class="project-edit-fields" data-project-edit="${escapeHtml(o.id)}"><label class="field"><span>Project name</span><input data-project-title maxlength="100" value="${escapeHtml(o.title)}"></label><label class="field"><span>Description</span><textarea data-project-description maxlength="700">${escapeHtml(o.description)}</textarea></label><label class="field"><span>Tags</span><input data-project-tags maxlength="120" value="${escapeHtml((o.tags||[]).join(', '))}"></label><button class="primary-button" data-project-save type="button">Save project</button></div></details>`:''}</section>`;
}
async function grantProjectStatus(projectId,controls){
  const o=projectById(projectId);if(!o||!canManageProjectClient(o))return toast('You do not have Project management permission.');const target=$('[data-project-person]',controls)?.value||'',status=$('[data-project-status]',controls)?.value||'moderator',duration=$('[data-project-duration]',controls)?.value||'none',reason=($('[data-project-reason]',controls)?.value||'').trim().slice(0,240);if(!target)return toast('Choose a public profile.');
  const targetProfile=state.profiles[target];if(targetProfile?._stub)return toast('That public profile has not finished syncing yet.');const button=$('[data-project-grant]',controls),old=button?.textContent||'Grant project Status';if(button){button.disabled=true;button.textContent='Granting…';}
  try{const {db,fsMod}=state.firebase,scopeType='project',scopeId=projectId,assignmentId=statusDocIdClient(scopeType,scopeId,status,target),ref=fsMod.doc(db,'statusAssignments',assignmentId),existing=await fsMod.getDoc(ref);let expiresAt=null;if(duration!=='none'){const ms={hour:3600000,day:86400000,week:604800000}[duration]||0;expiresAt=fsMod.Timestamp.fromDate(new Date(Date.now()+ms));}const actionId=crypto.randomUUID();const assignment={profileId:target,status,scopeType,scopeId,visibility:status==='timeout'?'private':'public',active:true,expiresAt,reason:(status==='timeout'||status==='member')?reason:'',grantedByProfileId:state.profileId,createdAt:existing.exists()?existing.data().createdAt:fsMod.serverTimestamp(),updatedAt:fsMod.serverTimestamp(),revokedAt:null,revokedByProfileId:'',lastActionId:actionId};const log={actorProfileId:state.profileId,targetProfileId:target,targetCollection:'statusAssignments',targetId:assignmentId,action:'status_grant',scopeType,scopeId,reason,snapshot:{status,scopeType,scopeId,expiresAt:expiresAt||null},createdAt:fsMod.serverTimestamp()};const batch=fsMod.writeBatch(db);batch.set(fsMod.doc(db,'moderationLogs',actionId),log);batch.set(ref,assignment,{merge:false});await batch.commit();const snap=await fsMod.getDoc(ref);if(snap.exists()){const row={id:snap.id,...snap.data()};state.statusPublic=[row,...state.statusPublic.filter(x=>x.id!==row.id)];state.statusPrivileged=[row,...state.statusPrivileged.filter(x=>x.id!==row.id)];if(target===state.profileId)state.statusOwn=[row,...state.statusOwn.filter(x=>x.id!==row.id)];mergeStatusRows();}openObjectDetail(projectId);toast(`${status==='moderator'?'Project Moderator':status==='member'?'Project Member':'Project Timeout'} Status granted.`);}catch(error){console.error('project status grant failed',error);toast(firestoreErrorText(error,'grant this Project Status'));}finally{if(button){button.disabled=false;button.textContent=old;}}
}
async function saveProjectDetails(projectId,box){
  const o=projectById(projectId);if(!o||!canManageProjectClient(o))return toast('You do not have Project management permission.');const title=($('[data-project-title]',box)?.value||'').trim().replace(/\s+/g,' '),description=($('[data-project-description]',box)?.value||'').trim(),tags=parseTags($('[data-project-tags]',box)?.value||'');if(!title||!description)return toast('Project name and description are required.');const button=$('[data-project-save]',box),old=button?.textContent||'Save project';if(button){button.disabled=true;button.textContent='Saving…';}
  try{const {db,fsMod}=state.firebase;await fsMod.updateDoc(fsMod.doc(db,'publicObjects',projectId),{title:title.slice(0,100),description:description.slice(0,700),tags,updatedAt:fsMod.serverTimestamp()});state.objects=state.objects.map(x=>x.id===projectId?{...x,title:title.slice(0,100),description:description.slice(0,700),tags,updatedAt:Date.now()}:x);renderCatalogs();renderFeed();renderUniverse();renderSearchPanel();openObjectDetail(projectId);toast('Project updated.');}catch(error){console.error('project edit failed',error);toast(firestoreErrorText(error,'edit this project'));}finally{if(button){button.disabled=false;button.textContent=old;}}
}

function openObjectDetail(id) { const o=state.objects.find(x=>x.id===id)||state.moderationObjects.find(x=>x.id===id); if(!o)return; state.detail={type:'object',id}; const who=identity(o.authorProfileId); const rel=state.connections.filter(c=>c.sourceId===id||c.targetId===id); const canRemove=o.authorProfileId===state.profileId||canModerateObjectClient(o); const scope=o.kind==='project'?'project':'discussion_object'; $('#detailEyebrow').textContent=o.kind; $('#detailTitle').textContent=o.deleted?`Removed ${o.kind}`:o.title; $('#detailBody').innerHTML=`<div class="detail-author-row"><button class="identity-button" data-open-profile="${escapeHtml(o.authorProfileId)}" type="button">${avatarMarkup(who)}<span><b>${escapeHtml(who.displayName)}</b><span class="status-badge-row">${statusBadgeMarkup(o.authorProfileId,scope,id)}</span><small>${timeAgo(o.createdAt)}</small></span></button><button class="context-link-button" data-open-community="${escapeHtml(o.spaceId)}" type="button">${escapeHtml(spaceById(o.spaceId).name)}</button></div><p class="detail-main-copy">${escapeHtml(o.description)}</p>${o.deleted?`<div class="moderation-notice"><b>Removed from normal LCS views</b><span>${escapeHtml(o.deleteReason||'No reason recorded.')}</span></div>`:''}<div class="detail-tags tag-row">${(o.tags||[]).map(t=>`<span>${escapeHtml(t)}</span>`).join('')}</div><div class="detail-actions">${o.deleted?(canRemove?`<button class="ghost-button" data-content-restore="publicObjects" data-content-id="${escapeHtml(id)}" type="button">↺ Restore</button>`:''):`<button class="ghost-button ${isFollowing('object',id)?'active-action':''}" data-follow-type="object" data-follow-id="${escapeHtml(id)}" type="button">${isFollowing('object',id)?'★ Following':'☆ Follow'} · ${followCount('object',id)}</button><button class="ghost-button" data-connect-object="${escapeHtml(id)}" type="button">↗ Connect another item</button>${canRemove?`<button class="ghost-button danger-button" data-content-remove="publicObjects" data-content-id="${escapeHtml(id)}" type="button">${o.authorProfileId===state.profileId?'Delete my item':'Remove item'}</button>`:''}`}</div>${projectManagementMarkup(o)}<section class="linked-section"><h3>Relationships</h3>${rel.length?rel.map(c=>{const other=state.objects.find(x=>x.id===(c.sourceId===id?c.targetId:c.sourceId));return other?`<button class="linked-object" data-open-object="${escapeHtml(other.id)}" type="button"><span>${escapeHtml(c.relation)}</span><b>${escapeHtml(other.title)}</b><small>${escapeHtml(other.kind)}</small></button>`:'';}).join(''):'<p class="muted">No relationships yet.</p>'}</section>`; renderDetailThread(); startDetailCommentSubscription(); showDialog('#detailDialog'); }
...
function openChannelDialog(spaceId=''){if(!requireUser())return;renderChannelSelects(); if(!state.spaces.some(s=>canModerateCommunityClient(s))){toast('Create or join a community as a moderator first.');return;} if(spaceId)$('#channelCommunity').value=spaceId;showDialog('#channelDialog');}
...
function renderStatusSurfaces(){
  ...
  const nav=$('#moderationNav'); if(nav)nav.hidden=!canAccessModerationClient();
...
function renderStatusTargetOptions(){const profile=$('#statusTargetProfile');if(profile){const oldProfile=profile.value;const rows=Object.values(state.profiles).filter(p=>!p._stub).sort((a,b)=>String(a.displayName||'').localeCompare(String(b.displayName||'')));profile.innerHTML=rows.map(p=>`<option value="${escapeHtml(p.id)}">${escapeHtml(p.displayName||'Member')} · ${escapeHtml(publicIdShort(p.id))}</option>`).join('');if([...profile.options].some(o=>o.value===oldProfile))profile.value=oldProfile;}const kind=$('#statusScopeKind'),target=$('#statusScopeTarget');if(!kind||!target)return;const old=target.value,k=kind.value;if(k==='global'){target.innerHTML='<option value="_">Whole LCS network</option>';target.disabled=true;return;}target.disabled=false;if(k==='project'){const projects=state.objects.filter(o=>o.kind==='project'&&!o.deleted&&(isGlobalModerator()||canManageProjectClient(o)));target.innerHTML=projects.map(o=>`<option value="project|${escapeHtml(o.id)}">${escapeHtml(o.title)}${isProjectOwner(o)?' · creator':''}</option>`).join('')||'<option value="">No projects you manage</option>';if([...target.options].some(o=>o.value===old))target.value=old;return;}const posts=state.posts.map(p=>`<option value="discussion_post|${escapeHtml(p.id)}">Post · ${escapeHtml(p.text.slice(0,70))}</option>`);const objs=state.objects.map(o=>`<option value="discussion_object|${escapeHtml(o.id)}">${escapeHtml(o.kind)} · ${escapeHtml(o.title)}</option>`);target.innerHTML=[...posts,...objs].join('')||'<option value="">No discussions available</option>';if([...target.options].some(o=>o.value===old))target.value=old;}
function renderModeration(){const denied=$('#moderationDenied'),workspace=$('#moderationWorkspace');if(!denied||!workspace)return;const canSee=canAccessModerationClient();denied.hidden=canSee;workspace.hidden=!canSee;if(!canSee)return;const founder=isFounder(),globalMod=isGlobalModerator(),scopeKindEl=$('#statusScopeKind'),hasNonProjectModerator=activeStatusesFor().some(x=>x.status==='moderator'&&x.scopeType!=='project');if(scopeKindEl&&!globalMod&&!hasNonProjectModerator&&managedProjectIdsClient().length&&scopeKindEl.value!=='project')scopeKindEl.value='project';renderStatusTargetOptions();const kind=$('#statusScopeKind')?.value||'global',raw=$('#statusScopeTarget')?.value||'_',parts=kind==='global'?['global','_']:raw.split('|'),selectedScopeType=parts[0]||'',selectedScopeId=parts[1]||'',canProjectGrant=selectedScopeType==='project'&&canManageProjectClient(selectedScopeId);const statusSelect=$('#statusValue');if(statusSelect){[...statusSelect.options].forEach(o=>{o.hidden=(o.value==='moderator'&&!(founder||canProjectGrant))||(o.value==='member'&&!canProjectGrant);});if(statusSelect.selectedOptions[0]?.hidden)statusSelect.value='timeout';}const statuses=[...state.statuses].filter(isStatusActive).sort((a,b)=>String(a.status).localeCompare(String(b.status)));$('#activeStatusList').innerHTML=statuses.length?statuses.map(x=>{const p=identity(x.profileId);const rootProtected=x.status==='founder'&&x.profileId===rootFounderProfileId();const scopedManager=canModerateStatusScope(x.scopeType,x.scopeId);const revoke=!rootProtected&&(founder||(x.status==='timeout'&&scopedManager)||((x.status==='moderator'||x.status==='member')&&x.scopeType==='project'&&scopedManager));return `<div class="moderation-row"><div><span class="status-badge status-${escapeHtml(x.status)}">${STATUS_META[x.status]?.symbol||'•'} ${escapeHtml(STATUS_META[x.status]?.label||x.status)}</span><b>${escapeHtml(p.displayName)}</b><small>${escapeHtml(statusScopeDescription(x))}${x.expiresAt?` · expires ${new Date(timeValue(x.expiresAt)).toLocaleString()}`:''}</small></div><div class="request-actions">${revoke?`<button class="ghost-button danger-button" data-status-revoke="${escapeHtml(x.id)}" type="button">Remove status</button>`:`<button class="ghost-button" type="button" disabled>${rootProtected?'Root Founder protected':'No authority'}</button>`}</div></div>`;}).join(''):'<p class="muted">No active Status assignments visible in your moderation scope.</p>';
...
async function grantStatus(e){
  ...
  if(status==='founder'){toast('Founder is a single root authority configured in Firebase, not a grantable Status.');return;}
  if(!isFounder()&&!canModerateStatusScope(scopeType,scopeId)){toast('You do not manage that scope.');return;}if(status==='moderator'&&scopeType!=='project'&&!isFounder()){toast('Only the root Founder can grant Global or Discussion Moderator Status. Project managers can grant Project Moderator Status inside projects they manage.');return;}if(status==='member'&&(scopeType!=='project'||!canManageProjectClient(scopeId))){toast('Project Member Status can only be assigned inside a project you manage.');return;}
  if(isFounder())await ensureCanonicalFounderStatus();
...
async function revokeStatus(id){
  const row=state.statuses.find(x=>x.id===id);if(!row||!requireUser())return;
  if(row.status==='founder'&&row.profileId===rootFounderProfileId()){toast('The root Founder authority is protected. Remove erroneous Founder assignments from other profiles instead.');return;}
  const allowed=isFounder()||(row.status==='timeout'&&canModerateStatusScope(row.scopeType,row.scopeId))||((row.status==='moderator'||row.status==='member')&&row.scopeType==='project'&&canModerateStatusScope(row.scopeType,row.scopeId));if(!allowed){toast('You do not have authority to remove this Status.');return;}
...
async function ensureCanonicalFounderStatus(){
  ...
  try{
    await fsMod.setDoc(ref,{profileId:state.profileId,status:'founder',scopeType:'global',scopeId:'_',visibility:'public',active:true,expiresAt:null,reason:'',grantedByProfileId:state.profileId,createdAt:fsMod.serverTimestamp(),updatedAt:fsMod.serverTimestamp(),revokedAt:null,revokedByProfileId:'',lastActionId:'bootstrap'});
    const snap=await fsMod.getDoc(ref);if(snap.exists()){const row={id:snap.id,...snap.data()};state.statusOwn=[row,...state.statusOwn.filter(x=>x.id!==row.id)];state.statusPublic=[row,...state.statusPublic.filter(x=>x.id!==row.id)];mergeStatusRows();return true;}
  }catch(e){console.debug('Founder authority repair',e?.code||e);}
  return false;
}
...
const lastAuthCspViolation={blockedURI:'',directive:'',sourceFile:'',at:0};
window.addEventListener('securitypolicyviolation',event=>{
  const blocked=String(event?.blockedURI||'');
  if(!/(?:google|firebase|gstatic)/i.test(blocked))return;
  lastAuthCspViolation.blockedURI=blocked;
  lastAuthCspViolation.directive=String(event?.effectiveDirective||event?.violatedDirective||'');
  lastAuthCspViolation.sourceFile=String(event?.sourceFile||'');
  lastAuthCspViolation.at=Date.now();
  console.warn('LCS auth-related CSP block',lastAuthCspViolation);
});
...
function authDiagnosticText(err,attempt,probe){
  const lines=[`attempt: ${attempt||'unknown'}`,`origin: ${location.origin}`,`path: ${location.pathname}`,`authDomain: ${LCS_CONFIG.firebase?.authDomain||''}`,`projectId: ${LCS_CONFIG.firebase?.projectId||''}`,`mobileBrowser: ${isMobileAuthBrowser()?'yes':'no'}`,`online: ${navigator.onLine?'yes':'no'}`];
  if(probe){lines.push(`projectConfigProbe: ${probe.ok?'ok':'failed'}`,`probeHttpStatus: ${probe.httpStatus||0}`);if(probe.errorStatus)lines.push(`probeErrorStatus: ${probe.errorStatus}`);if(probe.currentDomainAuthorized!==null)lines.push(`authorizedDomain: ${probe.currentDomainAuthorized?'yes':'no'}`);}
  const message=compactAuthMessage(err?.message);if(message)lines.push(`firebaseMessage: ${message}`);
  if(lastAuthCspViolation.at&&Date.now()-lastAuthCspViolation.at<120000){lines.push(`lastCspBlockedUri: ${lastAuthCspViolation.blockedURI}`,`lastCspDirective: ${lastAuthCspViolation.directive}`);}
  return lines.join('\n');
}
...
function showAuthError(err,attempt='popup',probe=null){
  ...
  let text='Firebase could not complete Google sign-in.';
  ...
  else if(code.includes('popup-closed'))text='The Google sign-in window was closed before Firebase finished signing in.';
  else if(code.includes('popup-blocked'))text='The browser blocked the Google sign-in window. Allow popups for j12h36h.github.io and retry.';
  else if(code.includes('network-request-failed'))text='Firebase could not reach a required Google authentication endpoint. Check the connection and retry.';
  else if(code.includes('session-not-retained'))text='Google completed sign-in, but Firebase did not retain the authenticated session in this browser.';
  else if(code.includes('internal-error'))text='Firebase returned an internal Google sign-in error. Mobile now runs the exact same Firebase popup path as desktop; the diagnostic below will also report any Google/Firebase resource blocked by the page security policy.';
  ...
  const retry=$('#authRedirectRetryButton');if(retry)retry.hidden=false;
  $('#authErrorBox').hidden=false;
}
function openAuthDialog(){clearAuthError();const mount=$('#googleIdentityButtonMount');if(mount)mount.hidden=true;const fallback=$('#googleSignInButton');if(fallback)fallback.hidden=false;showDialog('#authDialog');}

async function waitForFirebaseAuthUser(auth,authMod,timeoutMs=5000){
...
async function signInGoogle(){
  ...
  if(mobile&&probe?.ok===false){showAuthError({code:'auth/project-config-request-failed',message:`Firebase project configuration request failed with HTTP ${probe.httpStatus}${probe.errorStatus?` (${probe.errorStatus})`:''}.`},'mobile-desktop-parity-preflight',probe);return;}
  if(mobile&&probe?.currentDomainAuthorized===false){showAuthError({code:'auth/unauthorized-domain',message:`${location.hostname} is not present in Firebase authorizedDomains.`},'mobile-desktop-parity-preflight',probe);return;}

  // Deliberately identical on desktop and mobile. No GIS, no custom OAuth redirect,
  // no Firebase-hosting bridge. The only mobile-specific work is diagnostics.
  ...
  try{
    ...
    const user=auth.currentUser||result?.user||await waitForFirebaseAuthUser(auth,authMod,mobile?7000:5000);
    if(!user){const e=new Error('Google returned to LCS, but Firebase did not retain the authenticated browser session.');e.code='auth/session-not-retained';throw e;}
    syncFirebaseAuthUser(user);
    if(state.firebaseReady)await ensurePrivateIdentityOnce();
    closeDialog('#authDialog');
    toast('Signed in. This browser will keep the LCS session until you sign out.');
  }catch(e){
    console.error('LCS Google popup sign-in',e);
    showAuthError(e,mobile?'mobile-desktop-parity-popup':'desktop-popup',probe);
  }
}
...
async function migrateLegacyOwnedData(){if(!state.authUid||!state.profileId)return;const {db,fsMod}=state.firebase;const pid=state.profileId,uid=state.authUid;
  const specs=[
    ...
    ['channels','ownerUid','publicChannels',d=>({spaceId:d.spaceId,name:d.name||'general',description:d.description||'',type:d.type||'discussion',ownerProfileId:pid,deleted:Boolean(d.deleted),deletedAt:null,deletedByProfileId:'',createdAt:d.createdAt||fsMod.serverTimestamp(),updatedAt:d.updatedAt||fsMod.serverTimestamp()})],
...
async function initFirebase(){
  ...
  try{
    ...
    // Google sign-in uses Firebase popup auth on both desktop and mobile.
    // This avoids GIS origin_mismatch and redirect third-party-storage handoff on GitHub Pages.
    ...
    publicSubscribe('publicProfiles',rows=>{state.profiles={...state.profiles,...Object.fromEntries(rows.map(p=>[p.id,{...p,_stub:false}]))};if(state.publicProfile)state.profiles[state.publicProfile.id]={...state.publicProfile,_stub:false};synthesizeReferencedProfiles();renderAuth();renderFeed();renderCatalogs();renderSearchPanel();renderConnections();renderLfg();renderStatusTargetOptions();renderActivity();hydrateReferencedProfiles().catch(console.debug);},{limit:1000});
    publicSubscribe('publicPosts',rows=>{state.posts=rows;synthesizeReferencedProfiles();renderFeed();renderCommunities();renderSearchPanel();renderConnections();renderStatusTargetOptions();renderActivity();hydrateReferencedProfiles().catch(console.debug);if(state.detail?.type==='post')openPostDetail(state.detail.id);},{orderBy:'createdAt',limit:250,filters:[['deleted','==',false]]});
    publicSubscribe('publicObjects',rows=>{state.objects=rows;synthesizeReferencedProfiles();renderCatalogs();renderUniverse();renderTrends();renderSearchPanel();renderConnections();renderStatusTargetOptions();renderActivity();renderStatusSurfaces();setupModerationSubscriptions();hydrateReferencedProfiles().catch(console.debug);if(state.detail?.type==='object')openObjectDetail(state.detail.id);},{orderBy:'createdAt',limit:350,filters:[['deleted','==',false]]});
    publicSubscribe('publicSpaces',rows=>{state.spaces=rows;renderSpaces();renderCommunities();renderFeed();renderCatalogs();renderSearchPanel();ensureOwnedCommunityMemberships().catch(console.debug);},{orderBy:'createdAt',limit:200});
    ...
    publicSubscribe('publicCommunityMembers',rows=>{state.communityMembers=rows;synthesizeReferencedProfiles();renderCommunities();if($('#communityDetailDialog')?.open)openCommunityDetail($('#communityEditForm')?.dataset.communityId||state.activeSpaceId);hydrateReferencedProfiles().catch(console.debug);ensureOwnedCommunityMemberships().catch(console.debug);},{orderBy:'joinedAt',limit:5000});
    publicSubscribe('publicComments',rows=>{state.comments=rows;renderDetailThread();renderFeed();renderCatalogs();renderActivity();},{orderBy:'createdAt',limit:2000,filters:[['deleted','==',false]]});
    publicSubscribe('publicReactions',rows=>{state.reactions=rows;renderFeed();renderCatalogs();renderActivity();},{limit:3000});
    publicSubscribe('publicFollows',rows=>{state.follows=rows;renderCatalogs();renderConnections();renderActivity();if(state.detail?.type==='profile')openProfileDetail(state.detail.id);},{limit:3000});
    ...
    setBackendStatus('LCS v0.9.16 Chat alerts + modal flow connected','Public content uses random public profile IDs. Status authorization, soft-delete retention, and immutable moderation logs are enforced by Firestore rules.','ok');
...
function bindUI(){
  ...
  $('#messagesButton')?.addEventListener('click',()=>openChats()); $('#openLogicGuide').addEventListener('click',()=>openLogicGuide()); $('#explainButton').addEventListener('click',()=>openLogicGuide()); $$('[data-guide]').forEach(b=>b.addEventListener('click',()=>openLogicGuide(b.dataset.guide)));
...
bindUI(); initialRender(); initFirebase(); setInterval(()=>{if(state.statuses.some(x=>x.expiresAt)){renderStatusSurfaces();renderAuth();renderDetailThread();renderActivity();setupModerationSubscriptions();}},30000);

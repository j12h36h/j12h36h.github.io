...
function renderChannelSelects() {
  ...
  const owned = state.spaces.filter(s => s.ownerProfileId === state.profileId || isFounder());
...
function canManageCommunityClient(space){return Boolean(space&&!space.system&&state.profileId&&(space.ownerProfileId===state.profileId||isFounder()));}
function renderCommunities() {
  const root=$('#communityCatalog'); if(!root)return;
  root.innerHTML=allSpaces().map(s=>{ const cs=channelsForSpace(s.id); const count=state.posts.filter(p=>p.spaceId===s.id&&!p.deleted).length+state.objects.filter(o=>o.spaceId===s.id&&!o.deleted).length; const canManage=canManageCommunityClient(s); return `<article class="community-card"><div class="community-card-head"><button class="community-profile-button" data-open-community="${escapeHtml(s.id)}" type="button"><p class="eyebrow">${s.system?'Network commons':'Public community'}</p><h2>${escapeHtml(s.name)}</h2><p>${escapeHtml(s.description||'No description yet.')}</p></button><div class="community-stats"><span><b>${cs.length}</b> channels</span><span><b>${count}</b> items</span></div></div><div class="community-channel-grid">${cs.map(c=>`<button class="community-channel-card" data-channel-filter="${escapeHtml(c.id)}" type="button"><span class="channel-symbol">${channelMeta(c.type).symbol}</span><span><b># ${escapeHtml(c.name)}</b><small>${escapeHtml(c.description||channelMeta(c.type).label)}</small></span><em>${state.posts.filter(p=>itemChannelId(p)===c.id&&!p.deleted).length}</em></button>`).join('')}</div>${canManage?`<div class="community-card-actions"><button class="ghost-button" data-open-community="${escapeHtml(s.id)}" type="button">✎ Edit community</button><button class="ghost-button" data-new-channel="${escapeHtml(s.id)}" type="button">＋ Add channel</button></div>`:''}</article>`;}).join('');
}
...
function openCommunityDetail(id){
  const space=spaceById(id);if(!space)return;const dialog=ensureCommunityDialog(),body=$('#communityDetailBody',dialog),canManage=canManageCommunityClient(space),channels=channelsForSpace(space.id),owner=space.system?null:identity(space.ownerProfileId);
  ...
  body.innerHTML=`<section class="community-profile-summary"><p>${escapeHtml(space.description||'No description yet.')}</p><div class="community-stats"><span><b>${channels.length}</b> channels</span><span><b>${items}</b> public items</span>${owner?`<span>Created by <b>${escapeHtml(owner.displayName)}</b></span>`:''}</div></section><div class="community-channel-grid">${channels.map(c=>`<button class="community-channel-card" data-channel-filter="${escapeHtml(c.id)}" type="button"><span class="channel-symbol">${channelMeta(c.type).symbol}</span><span><b># ${escapeHtml(c.name)}</b><small>${escapeHtml(c.description||channelMeta(c.type).label)}</small></span><em>${state.posts.filter(p=>itemChannelId(p)===c.id&&!p.deleted).length}</em></button>`).join('')}</div>${canManage?`<section class="community-manager-panel"><p class="eyebrow">Community management</p><form id="communityEditForm" data-community-id="${escapeHtml(space.id)}"><label class="field"><span>Name</span><input id="communityEditName" maxlength="50" minlength="2" value="${escapeHtml(space.name)}" required></label><label class="field"><span>Description</span><textarea id="communityEditDescription" maxlength="240">${escapeHtml(space.description||'')}</textarea></label><div class="modal-actions"><button class="ghost-button" data-new-channel="${escapeHtml(space.id)}" type="button">＋ Add channel</button><button class="primary-button" type="submit">Save community</button></div></form></section>`:''}`;
  showDialog(dialog);
}
async function saveCommunityEdit(e){
  e.preventDefault();if(!requireContribution())return;const form=e.currentTarget,id=form.dataset.communityId,space=state.spaces.find(x=>x.id===id);if(!space||!canManageCommunityClient(space))return toast('You do not have permission to edit this community.');
...
function openChannelDialog(spaceId=''){if(!requireUser())return;renderChannelSelects(); if(!state.spaces.some(s=>s.ownerProfileId===state.profileId||isFounder())){toast('Create a community first.');return;} if(spaceId)$('#channelCommunity').value=spaceId;showDialog('#channelDialog');}
...
async function createChannel(e){
  ...
  const spaceId=$('#channelCommunity').value;const space=state.spaces.find(s=>s.id===spaceId);if(!space||(space.ownerProfileId!==state.profileId&&!isFounder())){toast('You can only add channels to communities you manage.');return;}
  state.channelInFlight=true;if(button){button.disabled=true;button.textContent='Creating…';}
  try{const {db,fsMod}=state.firebase;await fsMod.addDoc(fsMod.collection(db,'publicChannels'),{spaceId,name:$('#channelName').value.trim().slice(0,40),description:$('#channelDescription').value.trim().slice(0,240),type:$('#channelType').value,ownerProfileId:space.ownerProfileId,createdAt:fsMod.serverTimestamp(),updatedAt:fsMod.serverTimestamp()});form.reset();closeDialog(form.closest('dialog'));toast('Channel created.');}

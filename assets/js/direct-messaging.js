...
const SAFETY_TEXT = 'Treat every direct message exactly like a public post or public message. Do not send your real name, address, phone/email, passwords, payment information, private account details, precise location, or anything you would not want publicly visible. Messages may be stored, reviewed, copied, shared, or become publicly accessible.';
...
export function createDirectMessenger({ db, fs, getCurrentProfileId, getProfile, listContacts, avatarMarkup = null, onError = console.error }) {
  if (!db || !fs) throw new Error('Direct messenger requires Firestore db + helpers.');
  const required=['doc','getDoc','setDoc','collection','query','where','onSnapshot','serverTimestamp','limit','orderBy'];
  for(const key of required) if(typeof fs[key]!=='function') throw new Error(`Direct messenger missing Firestore helper: ${key}`);
  const state={root:null,threadUnsub:null,messageUnsub:null,threads:[],contacts:[],profiles:new Map(),activeProfileId:'',messages:[],writingLanguage:browserLanguage(),targetLanguage:browserLanguage(),translationEnabled:false,translators:new Map(),busy:false};
  ...
  function buildRoot(){
    ...
    const root=document.createElement('div');root.className='eras-dm-root';root.hidden=true;
    root.innerHTML=`<section class="eras-dm-window" role="dialog" aria-modal="true" aria-label="Direct messages">
      <header class="eras-dm-header"><div class="eras-dm-header-copy"><b>DIRECT MESSAGES</b><span id="erasDmPeer">SELECT A CONNECTION</span></div><button class="eras-dm-close" type="button" aria-label="Close messages">×</button></header>
      <aside class="eras-dm-sidebar"><div class="eras-dm-sidebar-title">CONVERSATIONS + CONNECTIONS</div><div class="eras-dm-list" id="erasDmList"></div></aside>
      <main class="eras-dm-main"><div class="eras-dm-toolbar"><label for="erasDmWritingLanguage">I WRITE IN</label><select id="erasDmWritingLanguage">${languageOptions(state.writingLanguage)}</select><label for="erasDmTargetLanguage">TRANSLATE TO</label><select id="erasDmTargetLanguage">${languageOptions(state.targetLanguage)}</select><label class="eras-dm-translate-toggle"><input id="erasDmTranslateToggle" type="checkbox"> LIVE BROWSER TRANSLATION</label><span class="eras-dm-translation-status" id="erasDmTranslationStatus">Original message text is always shown. Translation stays in your browser when the browser Translator API is available.</span></div><div class="eras-dm-messages" id="erasDmMessages"><div class="eras-dm-no-thread">Choose an accepted connection to open a message thread.</div></div><form class="eras-dm-compose" id="erasDmCompose"><textarea id="erasDmText" maxlength="2000" placeholder="Write a direct message…" disabled></textarea><button class="eras-dm-send" type="submit" disabled>SEND</button></form></main>
      <footer class="eras-dm-safety"><b>DIRECT MESSAGE SAFETY:</b> ${html(SAFETY_TEXT)}</footer>
    </section>`;
  ...
  async function renderSidebar(){
    ...
    if(!ids.length){root.innerHTML='<div class="eras-dm-empty">No accepted connections yet. Add a friend in LCS before starting a direct message.</div>';return;}
    await Promise.all(ids.map(profile));
    root.innerHTML=ids.map(id=>{const p=state.profiles.get(id)||{displayName:'Member'},thread=threadMap.get(id),active=id===state.activeProfileId;return `<button class="eras-dm-person ${active?'is-active':''}" type="button" data-dm-profile="${html(id)}"><span class="eras-dm-person-avatar">${avatar(p)}</span><span class="eras-dm-person-copy"><b>${html(p.displayName||'Member')}</b><small>${html(thread?.lastMessagePreview||'Start a message')}</small></span></button>`;}).join('');
  }
  function startThreads(){
    ...
    state.threadUnsub=fs.onSnapshot(q,s=>{state.threads=s.docs.map(d=>({id:d.id,...d.data()})).sort((a,b)=>timeMs(b.updatedAt)-timeMs(a.updatedAt));renderSidebar();},e=>{onError(e);});
  }
  function stopThreads(){try{state.threadUnsub?.();}catch{}state.threadUnsub=null;}
  function stopMessages(){try{state.messageUnsub?.();}catch{}state.messageUnsub=null;state.messages=[];}
  async function ensureThread(otherId){
    const pid=currentId();if(!pid||!otherId||pid===otherId)throw new Error('Invalid direct-message participants.');
    const members=[pid,otherId].sort(),id=threadIdFor(pid,otherId),ref=fs.doc(db,'directMessageThreads',id),snap=await fs.getDoc(ref);
    if(!snap.exists())await fs.setDoc(ref,{members,createdAt:fs.serverTimestamp(),updatedAt:fs.serverTimestamp(),lastMessageAt:null,lastMessageSenderProfileId:'',lastMessagePreview:''});
    return {id,ref,members};
  }
  async function activateProfile(otherId){
    if(!otherId||otherId===currentId())return;buildRoot();state.activeProfileId=otherId;renderSidebar();const p=await profile(otherId);state.root.querySelector('#erasDmPeer').textContent=p?.displayName||'DIRECT MESSAGE';setComposeEnabled(true);stopMessages();
    try{const thread=await ensureThread(otherId);const q=fs.query(fs.collection(db,'directMessageThreads',thread.id,'messages'),fs.orderBy('createdAt','desc'),fs.limit(200));state.messageUnsub=fs.onSnapshot(q,s=>{state.messages=s.docs.map(d=>({id:d.id,...d.data()})).reverse();renderMessages();if(state.translationEnabled)prepareVisibleTranslations();},e=>{onError(e);state.root.querySelector('#erasDmMessages').innerHTML='<div class="eras-dm-no-thread">Unable to load this message thread.</div>';});}
    catch(e){onError(e);state.root.querySelector('#erasDmMessages').innerHTML='<div class="eras-dm-no-thread">This message thread could not be opened.</div>';setComposeEnabled(false);}
  }
  ...
  async function ensureTranslator(source,target){
    ...
    try{const availability=await globalThis.Translator.availability({sourceLanguage:source,targetLanguage:target});if(availability==='unavailable'){status(`Browser translation is unavailable for ${source.toUpperCase()} → ${target.toUpperCase()}. Original text remains available.`,true);return null;}const promise=globalThis.Translator.create({sourceLanguage:source,targetLanguage:target});state.translators.set(key,promise);const translator=await promise;state.translators.set(key,translator);status('Live translation ready. Translation is generated in-browser; original message text remains visible.');return translator;}
    catch(e){state.translators.delete(key);status('Browser translation could not start. Click “TRANSLATE” on a message to retry after a user interaction.',true);return null;}
  }
  ...
  async function renderMessages(){
    if(!state.root)return;const root=state.root.querySelector('#erasDmMessages');if(!root)return;if(!state.activeProfileId){root.innerHTML='<div class="eras-dm-no-thread">Choose an accepted connection to open a message thread.</div>';return;}if(!state.messages.length){root.innerHTML='<div class="eras-dm-no-thread">No messages yet. The safety notice below applies to every message in this window.</div>';return;}
  ...
  async function sendMessage(e){
    ...
    try{const thread=await ensureThread(state.activeProfileId),messageId=crypto.randomUUID(),messageRef=fs.doc(db,'directMessageThreads',thread.id,'messages',messageId);await fs.setDoc(messageRef,{senderProfileId:pid,recipientProfileId:state.activeProfileId,text:text.slice(0,2000),language:state.writingLanguage,createdAt:fs.serverTimestamp()});await fs.setDoc(thread.ref,{members:thread.members,updatedAt:fs.serverTimestamp(),lastMessageAt:fs.serverTimestamp(),lastMessageSenderProfileId:pid,lastMessagePreview:text.replace(/\s+/g,' ').slice(0,120)},{merge:true});textarea.value='';}
    catch(e2){onError(e2);status('Message could not be sent. Check your connection, account status, or friendship state.',true);}
    finally{state.busy=false;setComposeEnabled(true);textarea.focus();}
  }
  async function openInbox(){const root=buildRoot();root.hidden=false;startThreads();await loadContacts();if(state.activeProfileId)activateProfile(state.activeProfileId);}
